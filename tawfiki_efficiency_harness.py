...
SECRETS (via GCS Secret Manager)
=================================
  All secrets are fetched from GCS Secret Manager at runtime.
  gcs_project must be set in prereg.json.
...
def fetch_secret(project_id: str, secret_name: str) -> str:
    try:
        ...
        print(f'[FATAL] Failed to fetch secret {secret_name}: {ex}')
        sys.exit(1)
ENGINE_BIN  = None
ENGINE_KEY  = None
SALT        = None
SALT_SOURCE = 'gcs'
SEED        = None  # set in main() after GCS fetch
...
def control_incompressible(files_dir: str, nonce: str, prev: str) -> tuple[dict, str]:
    ...
    Fraud detection: byte-exact reconstruction must succeed on high-entropy input.
    A structural index engine must achieve reconstruction fidelity regardless of input entropy.
    Graded from grade_retention() on reconstruction output. Index size demoted to measurement.
    """
    ...
    retention = grade_retention(x, recon)
    _save(files_dir, "incompressible", x, recon or b"")
    return _make_control_record(
        "incompressible", retention,
        {"input_bytes":       in_b,
         "metric_B_bytes":    rec_b,
         "B_over_input":      round(rec_b / in_b, 4) if in_b else None,
         "retention":         retention,
         "note": "reconstruction fidelity on high-entropy input; FAIL = fraud"},
        nonce, prev
    )
...
def main():
    ...
    prereg_data  = json.loads(prereg_bytes.decode('utf-8'))
    gcs_project  = prereg_data['gcs_project']
    ENGINE_BIN   = fetch_secret(gcs_project, 'eff-engine-bin')
    ENGINE_KEY   = fetch_secret(gcs_project, 'eff-engine-key')
    SALT         = fetch_secret(gcs_project, 'eff-audit-salt')
    SEED         = hashlib.sha256(b'tee-harness-salt:' + SALT.encode()).digest()
    SALT_SOURCE  = 'gcs:eff-audit-salt'
...
ENGINE_KEY  = fetch_secret(PREREG_PROJECT, "eff-engine-key")
